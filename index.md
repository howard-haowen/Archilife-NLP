...
- __句子相似度__ - 使用餘弦相似性計算單一文本內的句子相似度，每個節點代表一個句子（共`16`）。後續導入`PageRank`，設定句子數量（例如`5`）後即獲得文本摘要
---
...
---
後來發現有`textrank4zh` 這個套件，專門處理中文摘要的問題。上面的方式需要事先斷句、斷詞，而`textrank4zh` 的好處是直接餵入原始文本，就能獲得關鍵詞、關鍵短語、摘要。上面的原始文本獲得的結果如下：
